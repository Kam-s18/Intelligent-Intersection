...
def value(l):
    coeff={'car':0.5,'truck':0.9, 'bus':0.8, 'bike':0.2, 'front':0.4, 'cycle':0.2}
    density=0
    for i in l:
        try:
            density+=coeff[i]
...
def repeat():
    global currentGreen, currentYellow, nextGreen, status0, status1, status2, status3, final_list
    while(signals[currentGreen].green>0):   # while the timer of current green signal is not zero
        #printStatus()
        #print(status0,status1,status2,status3)
        showStats()
        updateValues()
        time.sleep(1)
    ...
    den2=value(status2)
    den3=value(status3)
    d_den={0:den0,1:den1,2:den2,3:den3}
    l_den=list(d_den.values())
    maxi_den_lane=l_den.index(max(l_den))
    final_list=[0,0,0,0]
    final_list[maxi_den_lane]=1
    print(final_list)

    if maxi_den_lane==currentGreen:
        currentGreen=maxi_den_lane
        ...
        currentGreen = maxi_den_lane # set next signal as green signal
...
def initialVehicles():
    d={0:{"car":5,"bus":2,"truck":2,"bike":2},1:{"car":2,"bus":1,"truck":1,"bike":1},2:{"car":5,"bus":2,"truck":2,"bike":1},3:{"car":0,"bus":1,"truck":0,"bike":1}}
    for i in d:
        for j in d[i]:
            z=d[i][j]
            for k in range(z):
                ...
                Vehicle(lane_number, j, i, directionNumbers[i], will_turn)
                global status0,status1,status2,status3
                if i==0:
                    status0.append(j)
                elif i==1:
                    status1.append(j)
...
def generateVehicles():
    while(True):
        ...
        if(temp<dist[0]):
            direction_number = 0
            status0.append(vehicleTypes[vehicle_type])
        elif(temp<dist[1]):
            direction_number = 1
            status1.append(vehicleTypes[vehicle_type])
        elif(temp<dist[2]):
            direction_number = 2
            status2.append(vehicleTypes[vehicle_type])
        elif(temp<dist[3]):
            direction_number = 3
            status3.append(vehicleTypes[vehicle_type])
        Vehicle(lane_number, vehicleTypes[vehicle_type], direction_number, directionNumbers[direction_number], will_turn)
        time.sleep(1)

def showStats():
    totalVehicles = 0
    global status0,status1,status2,status3,mem_l
    print('Direction-wise Vehicle Counts')
    for i in range(0,4):
        if(signals[i]!=None):
            print('Direction',i+1,':',vehicles[directionNumbers[i]]['crossed'])
            curr=vehicles[directionNumbers[i]]['crossed']
            if i==0:
                if mem_l[i]==curr:
                    pass
                else:
                    status0=status0[vehicles[directionNumbers[i]]['crossed']:]
            elif i==1:
                if mem_l[i]==curr:
                    pass
                else:
                    status1=status1[vehicles[directionNumbers[i]]['crossed']:]
            elif i==2:
                if mem_l[i]==curr:
                    pass
                else:
                    status2=status2[vehicles[directionNumbers[i]]['crossed']:]
            elif i==3:
                if mem_l[i]==curr:
                    pass
                else:
                    status3=status3[vehicles[directionNumbers[i]]['crossed']:]
            mem_l[i]=vehicles[directionNumbers[i]]['crossed']
...
class Main:
    ...
    thread1a = threading.Thread(name="initialVehicles",target=initialVehicles, args=())    # Generating vehicles
    thread1a.daemon = True
    thread1a.start()
